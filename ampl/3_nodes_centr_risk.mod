...
param gamma {j in probas};
param alpha {i in nodes, j in probas};
param J_max {i in nodes};
...
var J {i in nodes, j in probas} >= 0;
...
minimize Total_cost:
	sum {i in nodes} (
        ...
        1/(1-chi[i]) * sum {j in probas} p[j]*u[i,j]
    );
...
subject to max_insurance {i in nodes, j in probas}: 
    J[i,j] <= J_max[i];
